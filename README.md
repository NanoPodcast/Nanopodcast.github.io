# NanoPod — Website (with real admin dashboard)

A full static site for the NanoPod school scientific club, backed by
...
- **Polish**: pop-in entrance animation on page load, scroll-reveal on cards
  and sections, an on-brand atom-spinner loading state, a "Why join us"
  member-quotes section on the homepage, and optional Firebase Analytics
  (page views + a handful of custom events — see Part 1, step 8).
...
- Give it any nickname (e.g. "NanoPod site")
...
without it. Takes about two minutes if you want it:

- **Firebase Console → Project settings (gear icon) → General tab**
- Scroll to **"Your apps" → your web app**. If you don't see a
  `measurementId` in the config there, click **Add Analytics** (or, in
  the left sidebar, go to **Analytics → Dashboard → Enable Google
  Analytics**) and follow the prompt — pick or create a Google Analytics
  account, keep the defaults.
- Once enabled, the web app's config block will include a line like
  `measurementId: "G-XXXXXXXXXX"`. Copy that value.
- Open **`js/firebase-init.js`** and paste it in:
  ```js
  measurementId: "G-XXXXXXXXXX"
  ```
- That's it — no other code changes needed. The site already loads the
  Analytics script on every public page (not on `admin.html`, so your
  own visits there don't skew the numbers) and will start sending:
  - automatic page views on every public page
  - `join_click` — whenever someone clicks a "Join" link/button
  - `join_submit` — a completed Join form submission
  - `lang_switch` — switching between EN / FR / AR
  - `memory_submit` — a memory submitted from the Memories page
  - `activity_form_submit` — a per-event sign-up/feedback form submission

  View all of this under **Analytics → Dashboard** (or **Realtime**,
  which updates within seconds — good for confirming it's wired up
  right after deploying). Data can take a few hours to show up in the
  main Dashboard the first time.

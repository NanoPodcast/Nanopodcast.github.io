  ...
  shareMemoryForm: function (container) {
    ...
    document.getElementById("memory-submit-form").addEventListener("submit", async function (e) {
      ...
      try {
        ...
        if (window.trackEvent) window.trackEvent("memory_submit");
      } catch (err) {
...
function buildActivityForm(eventId, formFields) {
  ...
  wrap.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      if (window.trackEvent) window.trackEvent("activity_form_submit", { activityId: eventId });
    } catch (err) {
      status.textContent = "Couldn't submit: " + err.message;
...
/* Marks newly-rendered cards for scroll-reveal (see initScrollReveal in script.js). */
function reveal(container) {
  if (window.initScrollReveal) window.initScrollReveal(container);
}
